...
def eval_rbbox_map(det_results,
    ...
    for i in range(num_classes):
        ...
        # 自定义假阳性
        false_postive = fp / np.maximum((tp + fp), eps)
        # calculate AP
        if scale_ranges is None:
            recalls = recalls[0, :]
            precisions = precisions[0, :]
            false_postive = false_postive[0, :]
...
def print_map_summary(mean_ap,
    ...
    # 假阳性率
    false_postive = np.zeros((num_scales, num_classes), dtype=np.float32)
        
    num_gts = np.zeros((num_scales, num_classes), dtype=int)
    for i, cls_result in enumerate(results):
        if cls_result['recall'].size > 0:
            recalls[:, i] = np.array(cls_result['recall'], ndmin=2)[:, -1]
            false_postive[:, i] = np.array(cls_result['false_postive'], ndmin=2)[:, -1]
        aps[:, i] = cls_result['ap']
    ...
    header = ['class', 'gts', 'dets', 'recall', 'false_postive','ap']
    for i in range(num_scales):
        if scale_ranges is not None:
        ...
        for j in range(num_classes):
            row_data = [
                ...
                f'{recalls[i, j]:.3f}', f'{false_postive[i, j]:.3f}',f'{aps[i, j]:.3f}'
            ]
