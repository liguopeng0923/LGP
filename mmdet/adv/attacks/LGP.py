    ...
    def forward(self,data) -> torch.Tensor:
        ...
        for step in range(self.num_iteration):
            if single_stage:
                ...
                bboxes = [bbox.reshape(-1,4) for bbox in bboxes_unique]
            ...
            #     out_file = osp.join("/remote-home/liguopeng/object_detection/object_detection/mmdetection/adv/test/bbox",img_meta['ori_filename'])
        
            #     imshow_det_bboxes(img,bboxes[0].reshape(-1,4).detach().cpu().numpy(),torch.argmax(scores[0],dim=-1).detach().cpu().numpy(),out_file=out_file,font_size=25)
            
            # The perceptibility limition of adversarial perturbation 
            adv_scores = torch.cat(scores,dim=0).to(device=device)
            adv_bboxes = torch.cat(bboxes,dim=0).to(device=device)[:,:4]
            ...
            iou_cost = [iouLoss(current_bboxes[i].reshape(-1,4),gt_bboxes[i].repeat_interleave(current_bboxes[i].shape[1],0).reshape(-1,4)) + regLoss(current_bboxes[i].reshape(-1,4)[:,:2],gt_bboxes[i].repeat_interleave(current_bboxes[i].shape[1],0).reshape(-1,4)[:,:2]) for i in range(batch)]
            ...
            for i in range(batch):
                ...
                mask2,_ = torch.sort(bbox_overlaps(gt_bboxes[i],bboxes[i],mode = "iou"),dim = -1,descending = True)
            ...
            # reg attack
            adv_bboxes_scale = bbox_xyxy_to_cxcywh(adv_bboxes.clone())
            ...
            attention_heatmap = foreground_weights_failed + background_weights
        ...
        model.mode = mode
        ious = bbox_overlaps(torch.cat(bboxes,dim=0),torch.cat(gt_bboxes,dim=0))
        iou50 = (ious > 0.5).sum()
        iou75 = (ious > 0.75).sum()
