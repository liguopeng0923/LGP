...
def filter_scores_and_topk(scores, score_thr, topk, results=None):
    ...
    """
    # 返回false 或者 true， 纬度与scores一致
    valid_mask = scores > score_thr
    # 返回下标为true的值，纬度为一维
    scores = scores[valid_mask]
    # 返回为true的下标，维度与scores一致
    valid_idxs = torch.nonzero(valid_mask)

    # 保留前topk个值
    num_topk = min(topk, valid_idxs.size(0))
    # torch.sort is actually faster than .topk (at least on GPUs)
    # 返回倒叙排列的scores，idxs为其在原始scores的下标
    scores, idxs = scores.sort(descending=True)
    # 保留前num_topk个值
    scores = scores[:num_topk]
    # 前num_topk个score在原始score的下标，维度与scores[:num_topk]一致
    topk_idxs = valid_idxs[idxs[:num_topk]]
    # unbid返回指定维度切片后的结果，
    # 调用之前topk_idxs代表符合条件的score在原来scores的位置，维度为（num_topk,2）,其中2代表着是第几个bbox的第几个label值最大（排除背景label）
    # 调用之后变为（2，num_topk）,第一维是需要保留下来的索引，第二维是标签
