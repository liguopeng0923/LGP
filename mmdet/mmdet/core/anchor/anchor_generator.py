...
# https://zhuanlan.zhihu.com/p/464204495

# anchor strides则有一点绕，我的理解是它指的是感受野的大小，也就是对应feature map对应到原图多少个像素，所以它的值等于原图中anchor的基础大小，这在代码中一般写作base_size，有了base_size，就可以得到一个基础框，此时一般使用((0, 0), (anchor_stride-1, anchor_stride-1))这个框
...
class AnchorGenerator:
    ...
    # 首先在初始化时，为anchor设置长宽，调用gen_base_anchors方法生成base_anchors，base_anchors总共len(num_levels)*len(scales)*len(ratios)个，即每个特征层包含len(scales)*len(ratios)个base_anchor。
    ...
    def single_level_grid_priors(self,
        ...
        """Generate grid anchors of a single level.
            利用特征图的大小加上base anchors, 得到每个特征图位置的对于原图的anchors
